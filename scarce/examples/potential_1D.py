''' Example that shows the potential in a planar sensor in 1D, even for a not fully depleted
    sensor. The poisson equation is solved numerically and compared to the analytical solution.
    The 1D solution is also correct in the 2D case for a sensor with 100% fill factor.

    The numerical solving for underdepleted sensors is not straight forward, due to the a priory
    unknown depletion depth. This example shows also how to deal with that.
'''
...
def calculate_potential(mesh, rho, epsilon, L, V_read, V_bias, x_dep):
    r''' Calculate the potential with a given space charge distribution and assumed depletion width.
    Of the depletion width is too large the resulting potential will have a minimum < bias voltage.
    This is unphysical.
    '''
...
def create_1D_planar_sensor():
    ...
    x_dep = silicon.get_depletion_depth(abs(V_bias), n_eff, temperature=300) * 1e6

    # Plot result
    plt.plot(X, potential_a, '--', linewidth=2, label='Analytic solution')
    plt.plot(np.array(mesh.getFaceCenters()[0, :]), np.array(potential.arithmeticFaceValue()), '-', label='Numeric solution')
    plt.plot([x_dep, x_dep], plt.ylim(), '--', linewidth=2., label='Depletion zone')
    plt.plot(plt.xlim(), [V_bias, V_bias], '--', linewidth=2., label='Bias voltage')
    plt.xlabel('Position [um]')
    plt.ylabel('Potential [V]')
    plt.grid()
    plt.legend(loc=0)
    plt.title('Potential of a %sfully depleted planar silicon sensor with full fill factor' % ('not ' if x_dep < L else ''))
    plt.show()
