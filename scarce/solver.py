...
class DriftDiffusionSolver(object):
    ...
    def solve(self, p0, q0, dt, n_steps=4000):
        ''' Solve the drift diffusion equation for quasi partciles and calculates the
            ...
            n_steps : number
                Number of steps in time. Has to be large enough that all particles
                reach the readout electrode.
        '''
...
#         print p_e.shape
        # Result arrays
        traj_e = np.zeros(shape=(n_steps, p_e.shape[0], p_e.shape[1]))   # Electron trajectories
        traj_h = np.zeros(shape=(n_steps, p_h.shape[0], p_h.shape[1]))   # Electron trajectories
        I_ind_e = np.zeros(shape=(n_steps, p_e.shape[1]))   # Current induced from electrons
        I_ind_h = np.zeros(shape=(n_steps, p_h.shape[1]))   # Current induced from holes
        ...
        def in_boundary(description, x, y):
            ...
            sel_x = np.logical_and(x >= description.min_x, x <= description.max_x)
            sel_y = np.logical_and(y >= description.min_y, y <= description.max_y)
        ...
        for step in range(n_steps):
            if not np.any(sel_e) and not np.any(sel_h):  # All particles out of boundary
                break
            ...
            # Weighting field in V/cm
            print 'p_e[0, sel_e]', p_e[0, sel_e]

            W_e = self.pot_w_descr.get_field(p_e[0, sel_e], p_e[1, sel_e]) * 1e4
            W_h = self.pot_w_descr.get_field(p_h[0, sel_h], p_h[1, sel_h]) * 1e4

            print 'W_e.shape', W_e.shape
            print 'v_e.shape', v_e.shape

            if np.any(p_e[0, sel_e]):
                I_ind_e[step, sel_e] = (W_e[0, 0] * v_e[0, 0] + W_e[1, 0] * v_e[1, 0]) * q0[sel_e] * dt

            if np.any(p_h[0, sel_h]):
                I_ind_h[step, sel_h] = (W_h[0, 0] * v_h[0, 0] + W_h[1, 0] * v_h[1, 0]) * q0[sel_h] * dt
            ...
            sel_e = in_boundary(description=self.pot_descr, x=p_e[0, :], y=p_e[1, :])
            sel_h = in_boundary(description=self.pot_descr, x=p_h[0, :], y=p_h[1, :])
            p_e[:, ~sel_e] = np.nan
            p_h[:, ~sel_h] = np.nan
